"""
Evaluation / scoring script for semantic segmentation predictions.

Inputs:
- Ground truth directory: ./predict_output/test_ground_truths/
    ground_truth_{id}.npy
- Baseline predictions (e.g., UNet): ./predict_output/baseline_predictions/
    prediction_{id}.npy
- Ours predictions (e.g., SelfNet): ./predict_output/test_predictions/
    prediction_{id}.npy

Metrics:
- per-class IoU (Intersection over Union) computed from the confusion matrix.
- mIoU (mean IoU) over defect classes (classes 1..3 in this code).
- mPA (mean Pixel Accuracy) printed for reference.

Scoring:
- Compares IoU improvements vs baseline and computes a custom score per class.
"""
...
def per_class_iu(hist):
    ...
    Note:
    - This function prints IoU for defect classes only (classes 1..end),
...
def compute_mIoU(gt_dir, pred_dir):
    ...
    Naming convention expected in pred_dir:
    - prediction_{id}.npy  (this script extracts {id} by splitting on '_' once)

    The corresponding ground truth file is expected at:
    - ground_truth_{id}.npy
    """
    num_classes = 4
    ...
    # Extract ids from prediction filenames.
    npy_name_list = [f.split('_')[1].split('.')[0] for f in os.listdir(pred_dir) if f.endswith('.npy')]

    # Build paired file lists with the agreed naming convention.
    ...
    for ind in range(len(gt_npy_files)):
        ...
        # Sanity check: prediction and label must have the same number of pixels.
...
if __name__ == "__main__":
    # Directories used by predict.py and the ground-truth exporter.
    score = 0
    ####计算class IoU分数####
    pred_dir = './predict_output/test_predictions/' # 自己的模型!!!!!   baseline_predictions
    base_dir = './predict_output/baseline_predictions/'  #  unet    baseline_predictions

    gt_dir = './predict_output/test_ground_truths/'
    ...
    # Custom scoring: reward IoU improvements over a threshold per defect class.
